##cross-validataion for Group coordinate descent : GCD least squares
##function arguments##
# x:data matrix
...
tune_pplssvm <- function(x, y, d, H, C, lambda.grid, gamma, penalty, max.iter=100, n.fold)
{
  require(grpreg)
  require(Matrix)
  require(expm)
  ...
  for (ll in 1:L){
    for (jj in 1:n.fold){
      ...
      #G.C.D.
      obj <- pplssvm(x=x[tr.id,,drop = F], y=y[tr.id,drop = F], H=H, C=C, lambda=lambda.grid[ll], gamma, penalty, max.iter=max.iter)$vectors[,1:d,drop = F]
