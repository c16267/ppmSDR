##################################################################
#max-SCAD penalized sparse SDR with logit via D.C. algorithm
##################################################################
SCADPlogit <- function(x, y, H, C, lambda, tol = 1.0e-4, maxiter) 
{
  n <- nrow(x)
  p <- ncol(x)
  ...
  W <- W0 <- Diagonal(x=pi * (1- pi))
  u.tilde <- u0.tilde <- as.vector(X.tilde %*% theta + diag(x=1/(pi * (1- pi))) %*% (u * pi))
  ...
  {
    # Dmat
    temp <- (t(X.tilde) %*% W) %*% X.tilde
    temp.D <- C/n * temp + S
    Dmat <- matrix(0, h*(p+1) + p, h*(p+1) + p)
    Dmat[1:(h*(p+1)), 1:(h*(p+1))] <- as.matrix(temp.D)
    
    Dmat <- Dmat + diag(x=rep(1.0e-10, h*(p+1) + p))
    
    # dvec
    dvec1 <- C/n * as.vector(t(u.tilde) %*% W %*% X.tilde)
    ...
    delta <- mean(abs(new.theta - theta))
    if (delta < tol) break
    
    theta <- new.theta
    pi <- as.vector(1/(1 + exp(c(u) * (X.tilde %*% theta))))
    W <- diag(x=pi * (1 - pi))
    u.tilde <- as.vector(X.tilde %*% theta + diag(x=1/(pi * (1- pi))) %*% (u * pi))
