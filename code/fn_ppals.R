  ...
  if(penalty == "grSCAD"){
    ...
    for(iter in 1:max.iter){
      ...
      for(j in seq_along(integer(J))){
        ...
        z_j <- (C/(2*n))*((t(G.tilde.big[,ind,drop=F])%*%pos.resid)+ (t(G.tilde.big.c[,ind,drop=F])%*%neg.resid)) + 2*init.theta[ind]; #z_j <- as.vector(z_j)
        z_norm <- norm(z_j, "2")
    ...
    for(iter in 1:max.iter){
      ...
      for(j in seq_along(integer(J))){
        ...
        z_j <- (C/(n))*((t(G.tilde.big[,ind,drop=F])%*%pos.resid)+ (t(G.tilde.big.c[,ind,drop=F])%*%neg.resid)) + 2*init.theta[ind]; #z_j <- as.vector(z_j)
        z_norm <- norm(z_j, "2")
        theta_j_new <- firm_thresholding(z_norm, lambda1=lambda, gamma=gamma)*z_j/z_norm
