# ---- penalized principal asymmetric least square regression ---- #
ppasls <- function(x, y, H, C, lambda, gamma, penalty, max.iter){
  n <- nrow(x)
  p <- ncol(x)
  ...
  if(penalty == "grSCAD"){
    ...
    for(iter in 1:max.iter){
      ...
      G.tilde.big <- as.matrix(a.eig$vectors %*% Diagonal(x=sqrt(a.eig$values)) %*% solve(a.eig$vectors))
      
      b.eig <- eigen(0.5*S+t(w.tilde.c)%*%w.tilde.c) 
      G.tilde.big.c <- as.matrix(b.eig$vectors %*% Diagonal(x=sqrt(b.eig$values)) %*% solve(b.eig$vectors))
      
      
      # temp_identity <- as.list(1:h)
      # for (k in 1:h)  temp_identity[[k]] <- diag(1.0e-5, p+1, p+1)
      # identity <- as.matrix(bdiag(temp_identity))
      # 
      # pseudo_G.tilde.big <- G.tilde.big + identity
      # G.tilde.big <- pseudo_G.tilde.big
      # 
      # pseudo_G.tilde.big.c <- G.tilde.big.c + identity
      # G.tilde.big.c <- pseudo_G.tilde.big.c
    ...
    for(iter in 1:max.iter){
      ...
      a.eig <- eigen(0.5*S+t(w.tilde)%*%w.tilde) 
      G.tilde.big <- (a.eig$vectors %*% Diagonal(x=sqrt(a.eig$values)) %*% solve(a.eig$vectors))
      
      b.eig <- eigen(0.5*S+t(w.tilde.c)%*%w.tilde.c) 
      G.tilde.big.c <- (b.eig$vectors %*% Diagonal(x=sqrt(b.eig$values)) %*% solve(b.eig$vectors))
