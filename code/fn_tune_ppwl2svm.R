...
tune.ppwl2svm <- function(x, y, d, H, C, lambda.grid, gamma, penalty, max.iter, n.fold)
{
  require(energy)
  n <- length(y)
  #p <- length(x)/n
  ...
  for (ll in 1:L){
    for (jj in 1:n.fold){
      ...
      #G.C.D.
      obj <- ppwl2svm(x=x[tr.id,,drop = F], y=y[tr.id,drop = F], H=H, C=C, lambda=lambda.grid[ll], gamma, penalty, max.iter)$evectors[,1:d,drop = F]
