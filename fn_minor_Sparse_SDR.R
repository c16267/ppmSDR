...
#   else if (z==0) s <- 0
#   if (abs(z) <= lambda1) return(0)
#   else if (abs(z) <= (lambda1*(1+lambda2)+lambda1)) return(s*(abs(z)-lambda1)/(1+lambda2))
#   else if (abs(z) <= gamma*lambda1*(1+lambda2)) return(s*(abs(z)-gamma*lambda1/(gamma-1))/(1-1/(gamma-1)+lambda2))
#   else return(z/(1+lambda2))
# }
...
fn_orthonormalization <- function(X, y, g=group, m, ncolY=dim(y)[2], bilevel=FALSE, family="gaussian") {
