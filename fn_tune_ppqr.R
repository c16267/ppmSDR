tune.ppqr <- function(x, y, d, H, C, lambda.grid, gamma, penalty, max.iter, n.fold, tol)
{
  library(energy)
  n <- length(y)
  #p <- length(x)/n
  ...
  for (ll in 1:L){
    for (jj in 1:n.fold){
      ...
      #G.C.D.
      obj <- ppqr(x=x[tr.id,,drop = F], y=y[tr.id,drop = F], H=H, C=C, lambda=lambda.grid[ll], gamma, penalty, max.iter, tol=1.0e-5)$evectors[,1:d,drop = F]
